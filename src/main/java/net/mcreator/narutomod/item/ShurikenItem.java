...
import net.mcreator.narutomod.entity.renderer.ShurikenRenderer;
import net.mcreator.narutomod.NarutoModModElements;

import java.util.Random;

@NarutoModModElements.ModElement.Tag
public class ShurikenItem extends NarutoModModElements.ModElement {
	@ObjectHolder("naruto_mod:shuriken")
	...
	public ShurikenItem(NarutoModModElements instance) {
		super(instance, 1);
	...
	public static class ItemRanged extends Item {
		...
		public void onUsingTick(ItemStack itemstack, LivingEntity entityLiving, int count) {
			...
			if (!world.isRemote && entityLiving instanceof ServerPlayerEntity) {
				...
				if (true) {
					ItemStack stack = ShootableItem.getHeldAmmo(entity, e -> e.getItem() == ShurikenItem.block);
					if (stack == ItemStack.EMPTY) {
						for (int i = 0; i < entity.inventory.mainInventory.size(); i++) {
							ItemStack teststack = entity.inventory.mainInventory.get(i);
							if (teststack != null && teststack.getItem() == ShurikenItem.block) {
								stack = teststack;
								break;
							}
						}
					...
					if (entity.abilities.isCreativeMode || stack != ItemStack.EMPTY) {
						ArrowCustomEntity entityarrow = shoot(world, entity, random, 1f, 2.5, 2);
						itemstack.damageItem(1, entity, e -> e.sendBreakAnimation(entity.getActiveHand()));
						if (entity.abilities.isCreativeMode) {
							entityarrow.pickupStatus = AbstractArrowEntity.PickupStatus.CREATIVE_ONLY;
						} else {
							if (new ItemStack(ShurikenItem.block).isDamageable()) {
