...
package net.mcreator.narutomod.world.dimension;
...
import net.mcreator.narutomod.item.SasukeDimensionItem;
import net.mcreator.narutomod.block.OtsutsukiStoneBlock;
...
public class SasukeDimensionDimension extends NarutoModModElements.ModElement {
	@ObjectHolder("naruto_mod:sasuke_dimension_portal")
	public static final CustomPortalBlock portal = null;

	public SasukeDimensionDimension(NarutoModModElements instance) {
		super(instance, 29);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(OtsutsukiStoneBlock.block);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("naruto_mod:sasuke_biome")).getGenerationSettings()
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("naruto_mod:sasuke_dimension"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("sasuke_dimension_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("sasuke_dimension_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("sasuke_dimension_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new SasukeDimensionItem().setRegistryName("sasuke_dimension"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("naruto_mod:sasuke_dimension"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("naruto_mod:sasuke_dimension")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == OtsutsukiStoneBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? OtsutsukiStoneBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, OtsutsukiStoneBlock.block.getDefaultState(), 3);
					}
				}
			}
